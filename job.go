...
func ProcessJobs(cfg *config.Config) {
	...
	for _, job := range cfg.Jobs {
		...
		if err != nil {
			logrus.Fatalf("Job for VM alias '%s' failed: %v", job.VMAlias, err)
		}

		// If ensure_off is true, shut down the VM before processing operations.
		if job.EnsureOff {
			logrus.Printf("Ensuring VM '%s' is off", vmConfig.VMName)
			jobs.ShutdownVM(vmConfig, operator)
			logrus.Printf("VM '%s' shut down successfully.", vmConfig.VMName)
		}
		...
		for _, op := range job.Operations {
			...
			default:
				logrus.Fatalf("Job failed: unknown operation type: %s", op.Type)
			}
		}
	}
}
